...
exports.TEST_APK_PKG = exports.REQUIRED_PARAMS = exports.EspressoRunner = void 0;
...
const TEST_SERVER_ROOT = path_1.default.resolve(__dirname, '..', '..', 'espresso-server');
...
class EspressoProxy extends driver_1.JWProxy {
    async proxyCommand(url, method, body = null) {
        ...
        if (exited) {
            throw new driver_1.errors.InvalidContextError(`'${method} ${url}' cannot be proxied to Espresso server because ` +
...
class EspressoRunner {
    constructor(log, opts = {}) {
        ...
        const modServerName = support_1.fs.sanitizeName(`${TEST_APK_PKG}_${manifestPayload.version}_${this.appPackage}_${this.adb.curDeviceId}.apk`, { replacement: '-' });
        this.modServerPath = path_1.default.resolve(this.tmpDir, modServerName);
        ...
        if (opts.useKeystore && opts.keystorePath && opts.keystorePassword && opts.keyAlias && opts.keyPassword) {
            this.signingConfig = (0, server_builder_1.buildServerSigningConfig)({
    ...
    async installTestApk() {
        ...
        if (rebuild && await support_1.fs.exists(this.modServerPath)) {
            this.log.debug(`Deleting the obsolete Espresso server package '${this.modServerPath}'`);
            await support_1.fs.unlink(this.modServerPath);
        }
    ...
    async buildNewModServer() {
        ...
        if (this.espressoBuildConfig) {
            ...
            if (await support_1.fs.exists(this.espressoBuildConfig)) {
                this.log.info(`Loading the build configuration from '${this.espressoBuildConfig}'`);
                buildConfigurationStr = await support_1.fs.readFile(this.espressoBuildConfig, 'utf8');
            }
            else {
        ...
        const dirName = support_1.fs.sanitizeName(`espresso-server-${this.adb.curDeviceId}`, {
            replacement: '-',
        });
        const serverPath = path_1.default.resolve(this.tmpDir, dirName);
        ...
        await (0, support_1.mkdirp)(serverPath);
        this.log.debug(`Copying espresso server template from ('${TEST_SERVER_ROOT}' to '${serverPath}')`);
        await (0, utils_1.copyGradleProjectRecursively)(TEST_SERVER_ROOT, serverPath);
        this.log.debug('Bulding espresso server');
        await new server_builder_1.ServerBuilder(this.log, {
            serverPath, buildConfiguration,
            showGradleLog: this.showGradleLog,
            testAppPackage: this.appPackage,
            signingConfig: this.signingConfig
        }).build();
        const apkPath = path_1.default.resolve(serverPath, 'app', 'build', 'outputs', 'apk', 'androidTest', 'debug', 'app-debug-androidTest.apk');
        this.log.debug(`Copying built apk from '${apkPath}' to '${this.modServerPath}'`);
        await support_1.fs.copyFile(apkPath, this.modServerPath);
    }
    async cleanupSessionLeftovers() {
        ...
        try {
            ...
            if (activeSessionIds.length) {
                ...
                await bluebird_1.default.all(activeSessionIds.map((id) => (0, axios_1.default)({
    ...
    async startSession(caps) {
        ...
        this.instProcess.on('output', (stdout, stderr) => {
            ...
            if (lodash_1.default.isEmpty(line.trim())) {
...
//# sourceMappingURL=espresso-runner.js.map