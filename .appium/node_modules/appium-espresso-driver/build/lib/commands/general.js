...
function requireOptions(options, requiredOptionNames) {
    if (!lodash_1.default.isArray(requiredOptionNames)) {
        requiredOptionNames = [requiredOptionNames];
    }
    const presentOptionNames = lodash_1.default.keys(options);
    const missingOptionNames = lodash_1.default.difference(requiredOptionNames, presentOptionNames);
    if (lodash_1.default.isEmpty(missingOptionNames)) {
        return options;
    }
...
commands.mobileIsToastVisible = async function mobileIsToastVisible(opts = {}) {
    ...
    if (!support_1.util.hasValue(text)) {
...
commands.mobileSetDate = async function mobileSetDate(opts = {}) {
    const { year, monthOfYear, dayOfMonth } = requireOptions(opts, ['year', 'monthOfYear', 'dayOfMonth']);
...
commands.getDisplayDensity = async function getDisplayDensity() {
    return /** @type {number} */ (await this.espresso.jwproxy.command('/appium/device/display_density', 'GET', {}));
};
/**
 *
 * @this {import('../driver').EspressoDriver}
...
commands.mobileUiautomatorPageSource = async function mobileUiautomatorPageSource() {
    return /** @type {string} */ (await this.espresso.jwproxy.command(`/appium/execute_mobile/uiautomator_page_source`, 'GET'));
};
/**
...
commands.getSettings = async function getSettings() {
    ...
    const serverSettings = /** @type {Record<String, any>} */ (await this.espresso.jwproxy.command(`/appium/settings`, 'GET'));
...
commands.mobileStartActivity = async function mobileStartActivity(opts) {
    ...
    const { appActivity, locale, optionalIntentArguments, optionalActivityArguments } = requireOptions(opts, ['appActivity']);
